    ...
    def test_order_by_date(self):
        ...
        fg.append_content_file(
            self.make_content_file(
                "litany", "the-litany", "The Litany", date=now - timedelta(hours=1)
            )
        )
        fg.append_content_file(
            self.make_content_file(
                "blog", "the-entry", "The Entry", date=now + timedelta(hours=1)
            )
        )
        fg.append_content_file(
            self.make_content_file("tutorial", "the-tutorial", "The Tutorial", date=now)
        )
        feed_xml = fg.generate_feed(CONFIG)
        parsed = feedparser.parse(feed_xml)
