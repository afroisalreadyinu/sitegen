...
class ContentTagTests(unittest.TestCase, CollectionTestBase):
    ...
    def test_get_context(self):
        ...
        ct.append_content_file(
            self.make_content_file("blog", "the-entry", "The Entry", date=date)
        )
        context = ct.get_context(CONFIG)
        assert "items" in context
        assert len(context.pop("items")) == 1
        assert context["page_content"] == PageContent(
            title="test",
            description="",
            canonical_url="http://bb.com/tag/test",
            date=date,
        )
        assert context["site_info"] == SiteInfo(
            site_name="HELLO", base_url="http://bb.com", section="tag"
        )

    def test_date_sorting(self):
        ct = ContentTag("blog")
        ct.append_content_file(
            self.make_content_file("blog", "top-content", "The Entry")
        )
        ct.append_content_file(
    ...
    def test_render_only_draft(self):
        ct = ContentTag("tech")
        ct.append_content_file(
            self.make_content_file("blog", "the-content", "The Entry", draft=True)
        )
    ...
    def test_render(self):
        ct = ContentTag("tech")
        ct.append_content_file(
            self.make_content_file("blog", "the-content", "The Entry")
        )
        templates = FakeTemplates([FakeTemplate("list.html")])
        rendered = ct.render(CONFIG, templates, self.workdir.name)
        output_path = Path(self.workdir.name) / "tag/tech/index.html"
...
class TagCollectionTests(unittest.TestCase, CollectionTestBase):
    ...
    def test_append_content_file(self):
        tc = TagCollection()
        cf = self.make_content_file(
            "blog", "the-entry", "The Entry", tags=["hello", "why-not"]
        )
    ...
    def test_get_context(self):
        ...
        date = datetime.now().replace(second=0, microsecond=0) + timedelta(hours=1)
        cf = self.make_content_file(
            "blog", "the-entry", "The Entry", tags=["hello", "why-not"], date=date
        )
        tc.append_content_file(cf)
        context = tc.get_context(CONFIG)
        assert context.pop("items") == list(tc.content_tags.values())
        assert context["page_content"] == PageContent(
            title="Tags", description="", canonical_url="http://bb.com/tag/", date=date
        )
        assert context["site_info"] == SiteInfo(
            site_name="HELLO", base_url="http://bb.com", section="tag"
        )
    ...
    def test_render_empty(self):
        ...
        templates = FakeTemplates([FakeTemplate("list.html")])
        tc.render(
            {"title": "The Blog", "baseurl": "http://bb.com"},
            templates,
            self.workdir.name,
        )
