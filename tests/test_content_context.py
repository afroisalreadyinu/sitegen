...
class ContentContextTests(unittest.TestCase, CollectionTestBase):
    ...
    def test_add_content_existing_section(self):
        context = ContentContext()
        context.add_content_file(
            self.make_content_file("blog", "the-entry", "The Entry")
        )
        context.add_content_file(
    ...
    def test_add_content_with_tag(self):
        context = ContentContext()
        context.add_content_file(
            self.make_content_file("", "the-entry", "The Entry", tags=["one", "two"])
        )
        context.add_content_file(
            self.make_content_file(
                "", "other-entry", "Other Entry", tags=["two", "three"]
            )
        )
    ...
    def test_load_directory(self):
        ...
        assert sorted(x.name for x in context.content_files) == [
            "blog-entry.md",
            "index.md",
        ]
    ...
    def test_skip_draft(self):
        ...
        context.add_content_file(
            self.make_content_file(
                "blog", "the-entry", "The Entry", tags=["one", "two"], draft=True
            )
        )
